(function($) {
  ...
  ml.calculate = function(obj, options, count, wysiwyg, getter, setter) {
    ...
    if (count == undefined) {
      if (options.truncateHtml) {
        count = ml.remove_lineending(ml.strip_tags(obj.val())).length;
      }
      else {
        count = ml.remove_lineending(obj.val()).length;
      }
    }
  ...
  ml.remove_lineending = function(str) {
    return str.replace(/(\r\n|\r|\n)/g, "");
  };
  ...
  ml.ckeditor = function() {
    // We only run it once
    var onlyOnce = false;
    if (!onlyOnce && typeof(CKEDITOR) != 'undefined') {
      onlyOnce = true;
  ...
  ml.ckeditorChange = function(e) {
    ...
    if (options.truncateHtml){
      ml.calculate($('#' + e.editor.element.getId()), options, ml.remove_lineending(ml.strip_tags(ml.ckeditorGetData(e))).length, e, 'ckeditorGetData', 'ckeditorSetData');
    }
    else {
      ml.calculate($('#' + e.editor.element.getId()), options, ml.remove_lineending(ml.ckeditorGetData(e)).length, e, 'ckeditorGetData', 'ckeditorSetData');
    }
  };
