 ...
 * Log function.
 */
site_ajax.log = function(text, data) {
 ...
 */
Drupal.ajax.prototype.commands.site_common_ajax = function (ajax, command, status) {

  // Invoke ajax as specified in command.
  if (command.type == 'view') {
    site_ajax.ajax_view(command.key);
  }
  else if (command.type == 'form') {
    site_ajax.ajax_form(command.key);
  }
  else if (command.type == 'call') {
    site_ajax.ajax_call(command.name, command.key, command.data, command.method, command.spam);
  }
}

/**
 * Custom commant do log info.
 */
Drupal.ajax.prototype.commands.site_common_log = function (ajax, command, status) {

  // Invoke ajax as specified in command.
  site_ajax.log(command.title, command.data);
}
...
Drupal.ajax.prototype.specifiedResponse = function() {
  ...
  $(document).trigger('site_ajax_start', [ajax.element_settings]);
...
Drupal.ajax.prototype._errorNew = function (response, uri) {
  site_ajax.log(Drupal.ajaxError(response, uri));
...
Drupal.ajax.prototype._successNew = function(arg1, arg2, arg3, arg4) {
  ...
  $(document).trigger('site_ajax_success', [ajax.element_settings, arg1]);
  ...
  for (i in arg1) {
    ...
    if (arg1[i].timeout) {
      ...
      setTimeout(function(command) {
        site_ajax.process_commands([command]);
  ...
  if (ajax.element_settings.fingerid && site_ajax._fingerprints[ajax.element_settings.fingerid]) {
    delete site_ajax._fingerprints[ajax.element_settings.fingerid];
  }

  // Pass this to all child windows.
  if (spreadCommands.length > 0) {
    for (name in site_ajax._windows) {
      var window_item = site_ajax._windows[name];
      if (window_item['window'].location && window_item['window'].site_ajax && window_item['window']._site_ajax_window_fingerprint) {
        if (window_item['window']._site_ajax_window_fingerprint == window_item['fingerprint']) {
          window_item['window'].site_ajax.process_commands(spreadCommands);
        }
      }
    }
  }
 ...
 */
site_ajax.process_commands = function (commands) {
  ...
  site_ajax.log('manual commands:', commands);
}

/**
 * Process queue.
 */
site_ajax.queue_process = function (_fingerid, _fingerprint, info, msg) {

  // Ad item to queue if specified.
  if (_fingerid && info) {
    site_ajax._queue[_fingerid] = {
  ...
  // Or attempt to call one of the queued items.
    ...
    for (_fingerid in site_ajax._queue) {
      // Load info.
      info = site_ajax._queue[_fingerid]['info'];
      _fingerprint =  site_ajax._queue[_fingerid]['_fingerprint'];

      // Check if we have newer data already send, then we discard this queue item.
      if (site_ajax._antispam[_fingerid] != _fingerprint) {
        delete site_ajax._queue[_fingerid];
        site_ajax.log('resend canceled: ', info.name);
      }
      // Do recall.
      if (site_ajax._queue[_fingerid]) {
        var result = site_ajax.ajax_call(info.name, info.url, info.data, info.method, info.spam, info.unlock, true);
        if (typeof(result) == 'string') {
          site_ajax.log('resend declined: ', result);
        }
        else {
          site_ajax.log('resend success: ', info.name);
          delete site_ajax._queue[_fingerid];
        }
      }
    }
  ...
  if (recheck) {
    setTimeout(function () {
      site_ajax.queue_process();
    }, 500);
  }
  site_ajax.log('ajax queue:', items.length + ' ' + items.join(', '));
}

/**
 * Simple ajax call wrapper.
 */
site_ajax.ajax_call = function (name, url, data, method, spam, unlock, skip_queue) {
  ...
  if (spam > 0 || unlock > 0) {
    ...
    if (site_ajax._fingerprints[_fingerid]) {
      if (_fingerprint - site_ajax._fingerprints[_fingerid] < unlock) {
        forbidden = 'locked:' + name;
      }
    }
    else {
      // If current request is finished we check for antispam.
      if (site_ajax._antispam[_fingerid]) {
        if (_fingerprint - site_ajax._antispam[_fingerid] < spam) {
  ...
  if (forbidden != '') {
    ...
    if (!skip_queue) {
      ...
      site_ajax.queue_process(_fingerid, site_ajax._antispam[_fingerid], {
        'name': name,
        'url': url,
        'data': data,
        'method': method,
  ...
  if (Drupal.ajax[name]) {
    ...
    site_ajax._fingerprints[_fingerid] = _fingerprint;
    site_ajax._antispam[_fingerid] = _fingerprint;
 ...
 */
site_ajax.view_settings = function (name, limited) {
 ...
 */
site_ajax.ajax_view = function (name, data, method, spam) {

  // Get view settings.
  data = data || site_ajax.view_settings(name);

  // Invoke ajax call.
  site_ajax.ajax_call(name, '/views/ajax', data, method, spam);
}

/**
 * Views ajax call wrapper.
 */
site_ajax.ajax_form = function (domID, spam) {
  var $form = $('form#' + domID);
  if ($form.length < 1) {
    // TODO log error.
    return;
  ...
  // Invoke ajax call.
  site_ajax.ajax_call(domID, url, data, method, spam);
}

/**
 * Connect new window to the ajax routine.
 */
site_ajax.window_set = function (new_window, name) {
  if (new_window) {
    new_window._site_ajax_window_fingerprint = new Date().getTime();
    site_ajax._windows[name] = {
      'window': new_window,
      'fingerprint': new_window._site_ajax_window_fingerprint
    };
  }
  else if (site_ajax._windows[name]) {
    delete site_ajax._windows[name];
  }
}
